-- Definitive authority roster load: service_role policy + definer SQL loader.

drop policy if exists "Service role reads room players" on public.room_players;
create policy "Service role reads room players"
  on public.room_players
  for select
  to service_role
  using (true);

drop policy if exists "Service role reads game rooms" on public.game_rooms;
create policy "Service role reads game rooms"
  on public.game_rooms
  for select
  to service_role
  using (true);

create or replace function public.load_authority_roster_players(p_room_id uuid)
returns jsonb
language sql
stable
security definer
...
revoke all on function public.load_authority_roster_players(uuid) from public;
grant execute on function public.load_authority_roster_players(uuid) to service_role;
...
begin
  ...
  if v_needs_roster then
    v_players := public.load_authority_roster_players(p_room_id);
...
drop function if exists public.hydrate_authority_roster(uuid);
